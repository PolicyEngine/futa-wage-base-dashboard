...
import { RESULTS, TEN_YEAR_TOTAL, MODEL_INFO, buildCsv } from '@/lib/data';
      ...
      <div className="max-w-5xl mx-auto px-4 py-8 space-y-6">
        ...
        <section className="bg-white rounded-lg shadow-md p-6">
          ...
          <div className="space-y-3 text-gray-700">
            <p>
              The Federal Unemployment Tax Act (FUTA) levies a 6.0% tax on the first $7,000 of
              each worker&apos;s annual wages, reduced to an effective 0.6% for employers that
              receive the full 5.4% credit for paying state unemployment taxes on time. The
              $7,000 taxable wage base has not changed since 1983.
            </p>
            <p>
              This dashboard estimates the federal revenue raised by increasing the wage base to
              $43,000 in 2026 and indexing it to the Consumer Price Index for All Urban Consumers
              (CPI-U) thereafter, rounded to the nearest $100, while holding the 6.0% rate and
              maximum 5.4% credit constant. Under CBO&apos;s inflation projections, the base
              reaches $53,600 by 2035.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-5">
              ...
              <p className="text-3xl font-bold text-gray-900 tabular-nums">$43,000 &rarr; $53,600</p>
        ...
        <section className="bg-white rounded-lg shadow-md p-6">
          ...
          {/* Year bubbles */}
          <div className="flex flex-wrap items-center gap-2 mb-6" role="tablist" aria-label="Select year">
            {RESULTS.map((r) => (
              <button
                key={r.year}
                role="tab"
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              ...
              <p className="text-xs text-gray-500 mt-1">
                ...
                {selected.year === 2026 ? ' (statutory start)' : ''}
              </p>
            </div>
            <div className="bg-gray-50 rounded-lg p-4">
              <p className="text-xs font-semibold uppercase tracking-wider text-gray-500 mb-1">
          ...
          <p className="text-sm text-gray-500 mb-6">
            {(selected.workersAbove7k / 1e6).toFixed(0)} million workers earn above the current
            $7,000 base in {selected.year}. The reform raises employer-side FUTA liability by an
            average of ${perWorker.toFixed(0)} per affected worker (maximum $
            {maxPerWorker.toFixed(0)} for a worker at or above the {formatDollars(selected.wageBase)}{' '}
            base).
          </p>
        ...
        <section className="bg-white rounded-lg shadow-md p-6">
          ...
          <div className="space-y-3 text-sm text-gray-700">
            <p>
              ...
              liability is computed per worker as min(gross wages, wage base) &times; 0.6%, the
              net rate assuming every employer receives the full 5.4% credit for state
              unemployment taxes.
            </p>
            <p>
              <strong>Indexing.</strong> The $43,000 base applies in 2026. Later years grow with
              the CPI-U using CBO&apos;s February 2026 projections, following the convention that
              the prior February&apos;s index sets the year&apos;s parameter, rounded to the
              nearest $100. The PolicyEngine web app does not expose parameter indexing, so this
              analysis scripts the Python model directly.
            </p>
            <p>
            ...
            <p>
              <strong>Benchmarking.</strong> The IRS Data Book reports actual FUTA collections
              of $7.9 billion in fiscal year 2023, $8.1 billion in 2024, and $8.8 billion in
              2025. The roughly $7 billion baseline shown here sits below those actuals by
              design, not by error: actual collections include credit-reduction surcharges
              &mdash; the extra 0.3 to 4.5 percentage points employers pay in states carrying
              unpaid federal UI loans (recently California, New York, and the U.S. Virgin
              Islands, about $1.7 billion per year) &mdash; plus penalties and interest. This
              dashboard instead holds every employer at the flat 0.6% net rate in both the
              baseline and the reform, so the additional-revenue figure isolates the wage-base
              change rather than loan-repayment dynamics that end when states repay. Two
              remaining modeling gaps roughly offset: the model does not exclude FUTA-exempt
              employment (government and 501(c)(3) employers), and it applies the wage base per
              worker rather than per employer.
            </p>
            <p>
          ...
          <ul className="mt-4 space-y-1 text-sm">
            {[
              ...
              {
                href: 'https://github.com/PolicyEngine/policyengine-us/pull/9326',
                text: 'PolicyEngine US PR #9326, FUTA credit-reduction rate corrections',
              },
            ].map(({ href, text }) => (
              <li key={href}>
                <a
